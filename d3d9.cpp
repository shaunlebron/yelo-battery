...
// Initialize the Menu when the device is created.
								   ...
								   IDirect3DDevice9 **ppReturnedDeviceInterface) 
{
	...
	*ppReturnedDeviceInterface = *temp;
	...
	Menu::Initialize(*ppReturnedDeviceInterface, pPresentationParameters);
	Fov::Initialize(pPresentationParameters);
	Camera::Initialize();
	return hr;
}

// Reset the font object when the device needs resetting
HRESULT f_IDirect3DDevice9::Reset(D3DPRESENT_PARAMETERS *pPresentationParameters) 
{
	...
	Menu::OnResetDevice(pPresentationParameters);
	Fov::OnResetDevice(pPresentationParameters);

	return hr;
}

// Draw the menu over the screen after everything in the game is drawn
